...
Expr DifferentiationVisitor::operator()(const DiagonalMatrix& x) const {
  return ExprFactory::diagonalMatrix(x.child->differentiate(var_));
}

// Transpose: d/dx(f(x)^T) = (d/dx(f(x)))^T
Expr DifferentiationVisitor::operator()(const Transpose& x) const {
  return ExprFactory::transpose(x.child->differentiate(var_));
}

// Negation: d/dx(-f(x)) = -d/dx(f(x))
Expr DifferentiationVisitor::operator()(const Negate& x) const {
...
Expr DifferentiationVisitor::operator()(const Log& x) const {
...
Expr DifferentiationVisitor::operator()(const Sum& x) const {
...
Expr DifferentiationVisitor::operator()(const Product& x) const {
