...
Expression::Expr getLagrangian() {
  ...
  auto lambda_g_term = negate(product({ lambda_g, sum({s, negate(g), negate(l_A)}) }));
  auto lambda_t_term = product({ lambda_t, sum({s, t, negate(u_A)}) });
  auto lambda_y_term = negate(product({ lambda_y, sum({x, negate(y), negate(l_x)}) }));
  auto lambda_z_term = product({ lambda_z, sum({x, z, negate(u_x)}) });

  auto lagrangian = sum({ xHx, cx, minusMuLogG, minusMuLogT, minusMuLogY, minusMuLogZ, lambda_A_term, lambda_g_term, lambda_t_term, lambda_y_term, lambda_z_term });
  return lagrangian;
}
...
int main() {
  ...
  auto expr2 = ExprFactory::sum({ ExprFactory::product({ x, y }), ExprFactory::product({ y, x }), ExprFactory::product({A,C}), ExprFactory::negate(ExprFactory::product({C, A })), ExprFactory::number(1.3), ExprFactory::number(2.3), ExprFactory::product({ExprFactory::number(1.3), x}), x, x});

  auto expr3 = ExprFactory::negate(ExprFactory::negate(ExprFactory::sum({ A, B })));
  ...
  std::cout << "Diff: " << expr2.simplify().differentiate(x).simplify().toString() << "\n";
  ...
  */
  ...
  for (auto& v : variables) {
    if (v.toString() == "g" || true) {
      ...
      std::cout << "diff " << v.toString() << ": " << diff.simplify().toString() << std::endl;
      auto k = ExprFactory::product({ v, diff });
      std::cout << "diff * v (" << v.toString() << "): " << k.simplify().toString() << std::endl;
    }
  }
